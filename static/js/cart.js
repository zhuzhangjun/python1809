		...
		if (arr) {
			...
			for (var i=0; i<arr.length; i++) {
				...
				$(' <tr><th colspan=" ' + 20 + ' "><div class="ncc-store-name">店铺：<a href="#"> '
      				+ obj.shopname + ' </a></div></th></tr> ').appendTo("#list");
				...
				$(' <td><a href="#" class="ncc-goods-thumb"><img src=" ' + obj.imgsrc + ' "></a></td> ').appendTo(shopList);	
	            $(' <td><a href="#" class="goods-title">' + obj.title + '</a><dt> ').appendTo(shopList);
	            $(' <td><em class="goods-price">'+ obj.price + '</em></td> ').appendTo(shopList); 
	            
	            $(' <td class="ws0"><a href="#" class="reducekey tip">-</a><input value=" ' 
	                + obj.num + ' " class="text w20" type="text"><a href="#" class="addkey tip">+</a></td>').appendTo(shopList);
	            //单个总价
	            $(' <td><em class="goods-subtotal"> '+ obj.price * obj.num + '</em></td> ').appendTo(shopList);
